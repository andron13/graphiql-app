        ...
        <ul className="space-y-4">
          {links.map((link) => (
            <li key={link.name}>
              <Link
                to={link.to}
                className="block rounded-md border border-transparent bg-blue-100 px-4 py-2 font-medium text-blue-600 transition-colors duration-200 hover:border-blue-300 hover:bg-blue-200 hover:text-blue-800"
                title={link.name}
              >
                {link.name}
              </Link>
